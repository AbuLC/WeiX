...
void WCgmBuLC::GMmeun(WeiXBuLC* w)
{
Start:
    system("cls");
    int i;
    ...
    case 4:
        ...
        for (j = 0; j < groupcontainer.size(); j++) {
            if (groupcontainer[j].gname == w->wgroup[i - 1] && groupcontainer[j].gtype == 2) {
                break;
            }
        }
...
void WCgmBuLC::newGroup(WeiXBuLC* w)
{
    system("cls");
Start:
    cout << "请输入您要创建微信群的名称：" << endl;
    string gm;
    cin >> gm;
    bool flag = true;
    for (int i = 0; i < groupcontainer.size(); i++) {
        if (gm == groupcontainer[i].gname && groupcontainer[i].gtype == 2) {
            flag = false;
        }
    }
    if (!flag) {
...
void WCgmBuLC::joinGroup(WeiXBuLC* w)
{
    system("cls");
    cout << "微信采用推荐制加群" << endl;
    cout << "您的好友：" << endl;
    ...
    int j;//群的位置
    for (j = 0; j < groupcontainer.size(); j++) {
        if (groupcontainer[j].gname == w->wgroup[n] && groupcontainer[j].gtype == 2) {
            break;
        }
    }
    groupcontainer[j].gpersonacc.push_back(w->wfriend[m]);
    int k = checkIndex(w->wfriend[m], 2);
...
void WCgmBuLC::quitGroup(WeiXBuLC* w)
{
    system("cls");
    allGroup(w);
    int i;
    ...
    int j;//记录群的位置
    for (j = 0; j < groupcontainer.size(); j++) {
        if (w->wgroup[i] == groupcontainer[j].gname && groupcontainer[j].gtype == 2) {
            break;
        }
    }
...
void WCgmBuLC::deletePerson(int index, int acc)
{
...
void WCgmBuLC::manageGroup(int index, WeiXBuLC* w)
{
Start:
    system("cls");
