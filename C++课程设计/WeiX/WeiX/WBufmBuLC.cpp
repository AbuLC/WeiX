#include"WBufmBuLC.h"
void WBufmBuLC::FMmeun(WeiXBuLC* w)
{
Start:
	system("cls");
	allFriend(w);
...
void WBufmBuLC::addNewFriend(WeiXBuLC* w)
{
Start:
	int friendacc;
	cout << "请输入要添加好友的账号：" << endl;
	cin >> friendacc;
	int index = checkIndex(friendacc, 4);
	if (index == -1) {
		cout << "账号输入错误！" << endl;
		system("pause");
		goto Start;
...
void WBufmBuLC::addNewFriend2(WeiXBuLC* w)
{
	system("cls");
	...
	if (personcontainer[checkPerson(w->showcustomerid())].showQQid() == 0) {
		...
		for (int i = 0; i < weixcontainer[index]->wfriend.size(); i++) {
			...
			//判断该好友是否开通微商账号
			if (personcontainer[checkPerson(weixcontainer[k]->showcustomerid())].showWeiBusinessid() != 0) {
				//判断是否已经添加
				if (!findIn(personcontainer[checkPerson(weixcontainer[k]->showcustomerid())].showWeiBusinessid(), w->wfriend)) {
					newfriend.push_back(personcontainer[checkPerson(weixcontainer[k]->showcustomerid())].showWeiBusinessid());
					count++;
	...
	int j;
	j = checkIndex(newfriend[n], 4);//好友账号操作
...
void WBufmBuLC::deleteFriend(WeiXBuLC* w)
{
	int n;
	...
	//对好友账号操作
	{
		int index = checkIndex(w->wfriend[n], 4);
...
void WBufmBuLC::allFriend(WeiXBuLC* w)
{
	system("cls");
	cout << w->showaccountnum() << "的好友列表：" << endl;
	for (int i = 0; i < w->wfriend.size(); i++) {
		cout << i+1 << "、" << w->wfriend[i] << " " << personcontainer[checkPerson(weixcontainer[checkIndex(w->wfriend[i], 4)]->showcustomerid())].showName() << endl;
	}
	cout << endl;
}
